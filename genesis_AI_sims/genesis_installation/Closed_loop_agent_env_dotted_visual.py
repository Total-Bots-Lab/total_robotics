...
# --- Closed-loop reference tracking with moving target and tip trace ---
Kp = 0.2
...
for step in range(num_steps):
    ...
    # Get and store tip position
    try:
        ee_pose = franka.get_link_pose("hand")
        ee_pos = ee_pose[:3] if hasattr(ee_pose, "__getitem__") else [0, 0, 0]
    except Exception:
        ee_pos = [0, 0, 0]
    tip_trace.append(ee_pos)

# --- Draw the actual tip trace as a blue path ---
scene.draw_debug_path(tip_trace)  # blue path
