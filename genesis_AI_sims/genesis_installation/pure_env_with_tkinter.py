...
# Initialize Genesis Native Dashboard - Replaces TensorBoard completely
log_print("\n� Initializing Genesis Native Dashboard (TensorBoard Replacement)...")

# GENESIS NATIVE DASHBOARD - Same data visualization as TensorBoard
USE_GENESIS_NATIVE_DASHBOARD = True  # Set to True to use Genesis Native instead of TensorBoard

if USE_GENESIS_NATIVE_DASHBOARD:
    # Genesis Native Dashboard with same data as TensorBoard
    try:
        from genesis_native_streamer import create_genesis_native_tensorboard_replacement
        
        # Initialize Genesis Native Dashboard with TensorBoard-compatible interface
        tensorboard_logger = create_genesis_native_tensorboard_replacement(
            save_dir="genesis_dashboard_logs",
            port=8090
        )
        
        # Configure data tracking (same as TensorBoard was tracking)
        config_data = {
            "algorithm": "DDPG",
            "robot": "Franka Panda", 
            "environment": "Genesis Physics",
            "experiment": "genesis_franka_rl",
            "learning_rate": 0.001,
            "batch_size": 64,
            "buffer_size": 100000,
            "gamma": 0.99,
        ...
        tensorboard_logger.log_hyperparameters(config_data)
        
        log_print(f"� Genesis Native Dashboard enabled!")
        log_print(f"🎬 PRIMARY: Genesis 3D Viewer - Real-time robot visualization")
        log_print(f"📊 SECONDARY: Web Dashboard - http://localhost:8090/dashboard.html")
        log_print(f"🔥 TensorBoard REPLACED with Genesis Native streaming!")
        log_print(f"📈 Tracking same metrics: rewards, errors, losses, actions")
        
    except ImportError:
        log_print("⚠️ Genesis Native Dashboard not available, using standard logging")
        USE_GENESIS_NATIVE_DASHBOARD = False

if not USE_GENESIS_NATIVE_DASHBOARD:
...
for episode in range(num_episodes):
    ...
    # Calculate episode statistics for console logging
...
# Calculate final training statistics manually
if len(episode_rewards) > 0:
    final_stats = {
        ...
        'total_rewards': {
            ...
            'final': float(episode_rewards[-1]),
            'improvement': float(episode_rewards[-1] - episode_rewards[0]) if len(episode_rewards) > 1 else 0
        }
    }
    
    # Save final statistics to file
    os.makedirs("training_metrics", exist_ok=True)
    summary_file = os.path.join("training_metrics", f"training_summary_{dt.now().strftime('%Y%m%d_%H%M%S')}.json")
    with open(summary_file, 'w') as f:
        json.dump(final_stats, f, indent=2)
    
    log_print(f"📊 FINAL TRAINING METRICS:")
    log_print(f"  Episodes Completed: {final_stats['episodes_completed']}")
    log_print(f"  Total Reward - Min: {final_stats['total_rewards']['min']:.2f}, Max: {final_stats['total_rewards']['max']:.2f}, Mean: {final_stats['total_rewards']['mean']:.2f}")
...
if final_stats:
    ...
    log_print(f"    - Improvement: {final_stats['total_rewards']['improvement']:.2f}")
    log_print(f"� Training summary saved in: training_metrics/ directory")
